...
train_df = df[df['Month_Val'] <= 6].copy()
val_df = df[df['Month_Val'] > 6].copy()

# Add historic target features: Last Known Credit Score (June credit score)
june_score = train_df[train_df['Month_Val'] == 6][['Customer_ID', 'target']].rename(columns={'target': 'Last_Known_Credit_Score'})
train_df = train_df.merge(june_score, on='Customer_ID', how='left')
val_df = val_df.merge(june_score, on='Customer_ID', how='left')

# In June score, since some customers might not have a record, let's fillna with median target
global_median_score = train_df['target'].median()
train_df['Last_Known_Credit_Score'] = train_df['Last_Known_Credit_Score'].fillna(global_median_score)
val_df['Last_Known_Credit_Score'] = val_df['Last_Known_Credit_Score'].fillna(global_median_score)

# Profile features computed ONLY on train_df
cols_to_profile = [
...
# For each customer, compute mean and std of these dynamic variables in the training period
train_profiles = train_df.groupby('Customer_ID')[cols_to_profile].agg(['mean', 'std'])
# Flatten columns
...
# Fillna for std (e.g. if only 1 value per customer)
...
numeric_features = [
    ...
    'History_Age_Months_Imputed', 'Last_Known_Credit_Score'
]

profile_cols = [f"{col}_{stat}" for col in cols_to_profile for stat in ['mean', 'std']]

loan_features = ['Loan_' + loan.replace(' ', '_').replace('-', '_') for loan in loan_types]
encoded_cat_features = [col + '_encoded' for col in cat_cols]

features = numeric_features + profile_cols + loan_features + encoded_cat_features
...
print(f"\nAdvanced Validation Accuracy: {acc:.5f}")
print(f"Advanced Validation Macro F1: {macro_f1:.5f}")
