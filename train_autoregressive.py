...
# BEFORE SPLITTING: Sort by Customer_ID and Month_Val
...
# target lag feature: the previous month's target
df['target_lag1'] = df.groupby('Customer_ID')['target'].shift(1)

# Split train & validation
# We only train where target_lag1 is not null, so Month_Val from 2 to 6
train_df = df[(df['Month_Val'] >= 2) & (df['Month_Val'] <= 6)].copy()
val_df = df[df['Month_Val'] > 6].copy() # July (7) and August (8)

# Profile features computed ONLY on training partition (months 1-6)
cols_to_profile = [
    'Outstanding_Debt_clean', 'Delay_from_due_date_clean', 'Num_of_Delayed_Payment_clean',
    'Changed_Credit_Limit_clean', 'Num_Credit_Inquiries_clean', 'Monthly_Balance_clean'
]
...
numeric_features = [
    ...
    'History_Age_Months_Imputed', 'target_lag1'   # Target lag is included in training!
]
...
features = numeric_features + profile_cols + lag_features + loan_features + encoded_cat_features
...
# Setup a clean validation dataset for training early stopping
# For validation, we have July and August. We will evaluate by predicting July first, then using that prediction to predict August.
# First prepare validation matrices:
# We will execute the validation predictions sequentially.
# For early stopping during training, let's just use July's true values, but then evaluate August with the prediction!
# Or we can just train the model for 250 trees, which was around the best iteration previously.
# Let's train without early stopping on validation, but with a fixed number of rounds (e.g. 230).
params = {
...
# Sequential (auto-regressive) prediction for validation
print("Performing sequential validation...")
# Sort validation by Customer_ID, then Month_Val
val_df = val_df.sort_values(['Customer_ID', 'Month_Val']).reset_index(drop=True)

# We will predict month by month.
# First: month 7 (July). Its target_lag1 is the true June score, which is known!
# Second: month 8 (August). Its target_lag1 is the predicted July score!

# Filter July and August
...
# Map predictions to val_8's target_lag1!
# We can do this by merging or mapping. Let's create a dictionary:
pred_dict = dict(zip(val_7['Customer_ID'], val_7['pred']))
val_8['target_lag1'] = val_8['Customer_ID'].map(pred_dict).fillna(val_8['target_lag1']) # Fallback if missing
...
print(f"\nAuto-regressive Validation Accuracy: {acc:.5f}")
print(f"Auto-regressive Validation Macro F1: {macro_f1:.5f}")

print("\nClassification Report:")
print(classification_report(y_val_combined, y_pred_combined, target_names=['Poor', 'Standard', 'Good']))
