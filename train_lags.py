...
# BEFORE SPLITTING: Sort by Customer_ID and Month_Val to compute lags correctly
df = df.sort_values(['Customer_ID', 'Month_Val']).reset_index(drop=True)

# Compute lags and diffs
lag_cols = [
...
train_profiles = train_df.groupby('Customer_ID')[cols_to_profile].agg(['mean', 'std', 'min', 'max'])
...
print(f"\nLags-included Validation Accuracy: {acc:.5f}")
print(f"Lags-included Validation Macro F1: {macro_f1:.5f}")
